...
impl<'ctx> PartialEq for Type<'ctx> {
    fn eq(&self, other: &Self) -> bool {
        match self {
            Type::Unknown => false,
            Type::Never => match other { Type::Never => true, _ => false },
            Type::Void => match other { Type::Void => true, _ => false },
            Type::Bool => match other { Type::Bool => true, _ => false },
            Type::U8 => match other { Type::U8 => true, _ => false },
            Type::I8 => match other { Type::I8 => true, _ => false },
            Type::U16 => match other { Type::U16 => true, _ => false },
            Type::I16 => match other { Type::I16 => true, _ => false },
...
mod tests {
    ...
    fn eq() {
        ...
        let _s = Type::Struct(vec![
            StructMember { name: &"x", type_: &_u8, offset: 0 }
        ]);
        ...
        assert!(_s == _s);
        // nominal typing
        assert!(_s != Type::Struct(vec![
            StructMember { name: &"x", type_: &_u8, offset: 0 }
        ]));
        assert!(_f == _f);
        assert!(_f == Type::Func(vec![&Type::U8, &Type::Ptr(&Type::U8)], &Type::Array(&Type::U8, 10)));
        assert!(_f != Type::Func(vec![&Type::U8, &Type::Ptr(&Type::U16)], &Type::Array(&Type::U8, 9)));
    }
}
